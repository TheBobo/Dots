...
$(function () {
    ...
    signalRChatHub.client.broadcastUsers = function (name, message) {
        ...
        if ($('.' + className).length == 0 && myDeviceId != name) {
            ...
            $('#lobbyUsers').append('<div class="col col-xs-12 row-holder" > <div class="col col-xs-6">' + message + '</div>' +
                            '<div class="col col-xs-6"><input class="input-invite"  placeholder="enter amount" type="number" /></div></div>' +
                            '<div class="col col-xs-12"><p class="invite btn btn-success" style="" user-id="' + name + '">Invite</p></div>');
        }
    };
